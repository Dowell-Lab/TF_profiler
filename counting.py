...
class ListSequences:
    ...
    def list_sequences(self):
        fasta_sequences = self.outdir + "/generated_sequences/" + self.sample + "_experimental_window_sequences.fa"
...
# Child class (inherits from ListSequences() class)                      
class CountBases:

    def __init__(self, sequences, outdir, sample, windows=1500):
    ...
    def count_bases(self):
        '''calculate per position base composition across multiplesequences of even length.
        anew,tnew,cnew,gnew,nnew : list of lists normalized base counts for every position across all sequences'''
        #Checking sequence list
        first = [self.sequences[0:1]]
        last =  [self.sequences[-1:]]
        print('Sequence List: Contains ' + str(len(self.sequences)) + " elements. " + str(first) + '...' + str(last))
        
        #since the window is flanking a single base we accout for that by 2(w)+1
        sequence_length = int(2*int(self.windows)+1)
        print('Full Window Length: +/- ' + str(self.windows) + ' = ' + str(sequence_length))
        ...
        base_df.to_csv(self.outdir + '/generated_sequences/' + self.sample +'_base_content.tsv', sep='\t')
        print('Generated Positional Base Frequencies: ' + self.outdir + '/generated_sequences/' + self.sample +'_base_content.tsv')
