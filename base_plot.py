...
def base_plot(seq, plottitle, outdir, sample, whichQ, window=1500,q1=False, q4=False, q2_q3=False):
    '''plot line plot for each bases content across sequences for all/Q1/Q4 sequences on the same grid. Parameters                                                                                                plottitle : str - title for the plot represented as a string                                             q1 : bool - True or False inorder to plot Q1 sequences (default = False)                                 q4 : bool - True or False inorder to plot Q4 sequences (default = False)                                 Returns                                                                                                 line plot of choice (all sequences, Q1 sequences or Q4 sequences)'''
    ...
    if q2_q3:
        ...
        counts = CountBases(seq, outdir, sample, int(window)).count_bases()
    ...
    plt.savefig(outdir + '/generated_sequences/' + sample + '_BaseDistribution_'+str(whichQ)+'.png',bbox_inches='tight')
    plt.cla()
    ...
    plt.legend(bbox_to_anchor=(1.05, 1),fontsize = 25, loc=2, borderaxespad=0.)
    plt.suptitle(plottitle, fontsize=40, fontweight='bold')
    plt.savefig(outdir + '/generated_sequences/' + sample + '_SmoothedBaseDistribution_'+str(whichQ)+'.png',bbox_inches='tight')
    plt.cla()
    
    print('Generated Base Composition Plots: ' + outdir + '/generated_sequences/' + sample + '_SmoothedBaseDistribution_'+str(whichQ)+'.png and ' + sample + '_BaseDistribution_'+str(whichQ)+'.png')
